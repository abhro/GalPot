...
#ifndef Matrix_GalPot_
#define Matrix_GalPot_ 1

#include "Inline.h"
#include "Vector.h"

#ifdef TransposeMatrix_h
    template<class T, int, int> class transpose;
#endif
...
public:
   ...
   ~Matrix() {}
    ...
    Matrix&      operator-=           (const Matrix&);
#ifdef TransposeMatrix_h
    Matrix(const transpose<T,N1,N2>&) ;
    Matrix&      operator=            (const transpose<T,N1,N2>&);
    Matrix&      operator+=           (const transpose<T,N1,N2>&);
    Matrix&      operator-=           (const transpose<T,N1,N2>&);
#endif
...
template<class T, int N1, int N2>
inline ostream& operator<< (ostream& s, const Matrix<T,N1,N2>& M)
    {  int i,j;
      s << '\n';
      for(i=0; i<N1; i++) {
        s << "   " << M(i,0);
...
#ifdef TransposeMatrix_h
#include "TMatrix_def.h"

template<class T, int N, int M>
inline transpose<T,N,M> operator! (const Matrix<T,M,N>& A)
{ return transpose<T,N,M>(A); }
