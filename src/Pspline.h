...
////////////////////////////////////////////////////////////////////////////////
template<class S, class T>
void Derivs(S* const x[3], T*** const y[4], int const n[3])
...
// dy/dx0
    for(j=0; j<n[1]; j++)
        for(k=0; k<n[2]; k++) {
            for(i=0; i<n[0]; i++) z[i]=y[0][i][j][k];
            SplinedY(x[0],z,n[0],z1,&zero);
            for(i=0; i<n[0]; i++) y[1][i][j][k]=z1[i];
        }

// dy/dx1
    for(i=0; i<n[0]; i++)
        for(k=0; k<n[2]; k++) {
            for(j=0; j<n[1]; j++) z[j]=y[0][i][j][k];
            SplinedY(x[1],z,n[1],z1,&zero);
            for(j=0; j<n[1]; j++) y[2][i][j][k]=z1[j];
        }
